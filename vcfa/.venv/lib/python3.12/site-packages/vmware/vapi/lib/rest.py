...
class OperationRestMetadata(object):
    ...
    def __init__(self, http_method, url_template='',
                 ...
                 content_type=None, use_status_to_std_err_map=True):
        """
        ...
        :param content_type: Value for the Content-Type HTTP header of the
                             request
        :type  use_status_to_std_err_map: :class:`bool`
        :param use_status_to_std_err_map: Flag indicating whether to map error
                                          responses to vAPI standard errors
        """
        ...
        self.use_status_to_std_err_map = use_status_to_std_err_map
    ...
    def get_url_path(self, path_variable_fields, query_parameter_fields):
        ...
        for (field_name, field_values) in six.iteritems(query_parameter_fields):
            param_name = self._query_parameters[field_name]
            if isinstance(field_values, str):
                val = ('%s=%s' % (param_name, field_values))
                query_parameter_list.append(val)
            elif isinstance(field_values, list):
                for field_value in field_values:
                    val = ('%s=%s' % (param_name, field_value))
                    query_parameter_list.append(val)
        query_parameter_str = '&'.join(query_parameter_list)
