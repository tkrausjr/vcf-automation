...
try:
    ...
    logger.info('Can not load urllib3 module, disable connection pool and response compression')
...
class HttpRpcProvider(HTTPProvider):
    ...
    def __init__(self, ssl_args, url, disable_conn_pool=False, disable_compression=False):
        """
        ...
        :type disable_compression: :class: 'bool'
        :param disable_compression: disable accepting compressed response
        """
        ...
        global ACCEPT_COMPRESSED_RESPONSE
        # Compressed response supported only for connection type urllib3.PoolManager
        if self.uds or not use_connection_pool or disable_compression:
    ...
    def disconnect(self):
        """ disconnect """
        if use_connection_pool and not self.uds and self.manager is not None:
            self.manager.clear()
    ...
    def do_request(self, http_request):
        ...
        if request:
            ...
            try:
                ...
                if use_connection_pool and not self.uds:
