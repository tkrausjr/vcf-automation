...
class WsgiServer(ServerInterface):
    ...
    def serve_forever(self):
        ...
        if self._ssl_args:
            ssl_context = self._create_ssl_context(keyfile=self._ssl_args['keyfile'],
                                                  certfile=self._ssl_args['certfile'])
    ...
    @staticmethod
    def _create_ssl_context(keyfile=None,
                            certfile=None,
                            ca_certs=None):
        """
        Factory method for creating SSLContext copied from ssl.wrap_socket
        method, removed as of python 3.12.

        Rather than utilizing ssl.create_default.context, the current method
        is introduced for backwards compatibility.
        """
