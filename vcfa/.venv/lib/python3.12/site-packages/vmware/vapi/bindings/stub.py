...
from vmware.vapi.bindings.common import (
    raise_bindings_exception, NameToTypeResolver)
...
class ApiInterfaceStub(ApiInterface):  # pylint: disable=W0223
    ...
    @staticmethod
    def _get_response_status_code(runtime_data):
        if runtime_data is None:
            return

        http_response = runtime_data.http_response
    ...
    @staticmethod
    def _set_error_runtime_data(api_error, runtime_data):
        """
        :type  api_error: :class:`vmware.vapi.bindings.VapiError`
        :param api_error: vAPI error
        :type  runtime_data: :class:`vmware.vapi.core.ResultRuntimeData`
        :param runtime_data: Infrastructure info about the API response
        """
        status = ApiInterfaceStub._get_response_status_code(runtime_data)
        if status:
            api_error.set_status_code(status)

    def _get_error_type(self, method_result, op_info, rest_metadata):
        if getattr(rest_metadata, "use_status_to_std_err_map", True) or not self._use_rest:
            errors = op_info['errors']
            return errors.get(method_result.error.name)

        throws_clause_def = op_info.get('throws_clause', None)
        if throws_clause_def:
            for throws_clause in throws_clause_def.throws_clause_elements:
                if self._match_error_type(ApiInterfaceStub._get_response_status_code(method_result.runtime_data),
                                          throws_clause.status_codes):
                    return throws_clause.error_type

    def _match_error_type(self, status_code, expected_status_codes):
        if expected_status_codes and not (status_code in expected_status_codes):
            return False

        #TODO Add discriminator matching criteria
    ...
    def native_invoke(self, ctx, method_name, kwargs):
        ...
        for validator in validators:
            ...
            raise_bindings_exception(msg_list, data_val)
        ...
        if method_result.success():
            ...
            for validator in validators:
                ...
                raise_bindings_exception(msg_list, method_result.output)
            ...
            error_type = self._get_error_type(method_result,
                                              op_info,
                                              operation_rest_metadata)
            ...
            if error_type is None:
                ...
                raise vapi_error
            api_error = TypeConverter.convert_to_python(method_result.error,  # pylint: disable=E0702
                                                        error_type,
                                                        self._config.resolver,
                                                        self._rest_converter_mode)
            self._set_error_runtime_data(api_error, method_result.runtime_data)
            raise api_error
