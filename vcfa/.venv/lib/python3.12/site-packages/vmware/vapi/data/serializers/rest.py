...
from vmware.vapi.core import MethodResult, ResultRuntimeData
...
class RequestSerializer(object):
    ...
    def serialize_input(input_value, rest_metadata):
        ...
        for (field_name, field_val) in input_value.get_fields():
            if field_name in path_variable_field_names:
                ...
                field_obj = RequestSerializer._serialize_query_parameters(field_val)
                if field_obj is not None:
                    query_param_fields[field_name] = field_obj
            elif field_name in header_field_names:
                field_str = RequestSerializer._convert_to_string(field_val)
                if field_str is not None:
                    header_fields[header_field_names[field_name]] = field_str
    ...
    @staticmethod
    def _serialize_query_parameters(data_value):
        """
        Serializes data value to object. (`list`  or `str`) for use in Request URL

        :type  data_value: :class:`vmware.vapi.data.value.DataValue`
        :param data_value: Data value
        :rtype   :class:`str` | :class:`list` | :class:`NoneType`
        :return: Serialized string for use in Request URL or None
        """
        if isinstance(data_value, OptionalValue):
            if data_value.is_set():
                return RequestSerializer._serialize_query_parameters(data_value.value)
            else:
                return None
        elif isinstance(data_value, ListValue):
            return [RequestSerializer._convert_to_string(val)
             for val in data_value if val is not None]
        else:
            return RequestSerializer._convert_to_string(data_value)
...
class ResponseDeserializer(object):
    ...
    @staticmethod
    def _deserialize_error(deserialized_struct, status, rest_metadata=None):
        if getattr(rest_metadata, "use_status_to_std_err_map", True):
            return ErrorValue(
                name=http_to_vapi_error_map[status],
                values={
                    'messages': ListValue(),
                    'data': deserialized_struct,
                })

        return ErrorValue(deserialized_struct.name,
                          deserialized_struct._fields)


    @staticmethod
    def deserialize_response(http_response, is_vapi_rest, rest_metadata=None):
        """
        Deserialize the REST response.

        If the REST metadata for the API operation prescribes to map error responses to
        vAPI standard errors based on HTTP status code, then the deserialized ErrorValue
        is mapped to a vAPI standard error. Otherwise the ErrorValue represents directly
        the deserialized wire data.

        :type  http_response: :class:`vmware.vapi.protocol.client.http_lib.HTTPResponse`
        :param http_response: HTTP response of the API call
        :type  is_vapi_rest: :class:`bool`
        :param is_vapi_rest: Whether the Rest json message format is VAPI Rest
            or not
        :type  rest_metadata: :class:`vmware.vapi.lib.rest.OperationRestMetadata`
        :param rest_metadata: REST metadata associated with the operation call
        :rtype   :class:`vmware.vapi.core.MethodResult`
        :return: VAPI MethodResult
        """
        output, error = None, None
        if (http_response.body is not None
                and http_response.body != ''):
            response_value = DataValueConverter.convert_to_data_value(
                http_response.body)
            if http_response.status in successful_status_codes:
                ...
                if is_vapi_rest:
                    ...
                    error = ResponseDeserializer._deserialize_error(response_value,
            ...
            if http_response.status in successful_status_codes:
                ...
                if not is_vapi_rest:
                    error = ErrorValue(
                        name=http_to_vapi_error_map[http_response.status]
                    )
        return MethodResult(output=output, error=error, runtime_data=ResultRuntimeData(http_response))
...
class RestSerializer(object):
    ...
    def deserialize_response(http_response, is_vapi_rest, rest_metadata=None):
        ...
        return ResponseDeserializer.deserialize_response(
            http_response, is_vapi_rest, rest_metadata)
