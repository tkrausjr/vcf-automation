...
class RemovedComponents(VapiInterface):
    ...
    def get(self,
        ...
        :raise: :class:`com.vmware.vapi.std.errors_client.Unauthorized` 
            If the caller does not have the necessary privileges.
        :raise: :class:`com.vmware.vapi.std.errors_client.Unsupported` 
            If the host is not a vLCM managed standalone host. Unsupported type
            added in 9.0, previous releases may show InternalServerError.
...
class _EffectiveComponentsStub(ApiInterfaceStub):
    def __init__(self, config):
        ...
        list_with_removed_components_rest_metadata = OperationRestMetadata(
            http_method='GET',
            url_template='/esx/settings/hosts/{host}/software/drafts/{draft}/software/effective-components?with-removed-components',
            path_variables={
                'host': 'host',
                'draft': 'draft',
            },
